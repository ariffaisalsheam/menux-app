...
public interface OrderRepository extends JpaRepository<Order, UUID> {
     ...
     */
    @Query("SELECT o FROM Order o WHERE o.restaurant = :restaurant AND o.status IN ('PENDING', 'CONFIRMED', 'PREPARING') ORDER BY o.createdAt")
    List<Order> findPendingOrdersByRestaurant(@Param("restaurant") Restaurant restaurant);

    /**
     * Find today's orders by restaurant
     */
    @Query("SELECT o FROM Order o WHERE o.restaurant = :restaurant AND DATE(o.createdAt) = CURRENT_DATE ORDER BY o.createdAt DESC")
